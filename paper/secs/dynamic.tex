...
Since the KV pairs located in $loc$ and $loc+|h^i|/2$ of old subtable are hashed to $loc$ in the new table, there could be cases the KV pairs exceeds the size of a single bucket. 
Thus, we first assign a warp to accommodate KV pairs that can fit the size of a single bucket. Similarly, it does require locking since there will be no thread conflict on any bucket. 
...
We invoke Algorithm~\ref{algo:insert} to insert the residuals to the remaining two subtables. 
