...
For keys having size larger than 4 bytes, a simple strategy is to store less KV pairs in a bucket. For example, if the keys are 8-types, then a bucket can accommodate 16 KV pairs. In the worst case, a key taking 128 bytes occupy one bucket alone, which is unnecessary large in practice.
