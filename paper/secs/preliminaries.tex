...
The hash table offers the following functionalities: \formal{insert} - stores a KV or replaces an existing KV if the same key is found in the hash table; \formal{find} - given a key returns the associated value if it is stored, and NULL otherwise; and \formal{delete} - removes an existing KV if it presents in the table.  
