...
bool  test_size(TYPE size) {
    ...
    printf("insert锛歵he time is %.2lf us, ( %.2f Mops)\n",
    ...
    printf("search锛歵he time is %.2lf us ( %.2f Mops)\n",
           (double) diff, (double) (size) / diff);
    check_search_result(key,chck,value,size);

    checkCudaErrors(cudaGetLastError());

    printf("露露露test %d complete\n\n\n", size);
    checkCudaErrors(cudaGetLastError());
    delete[] key;
    delete[] value;
    delete[] search;
...
bool  test_size_remove(TYPE size) {
    ...
    timer.Stop();
    diff = timer.Elapsed()*1000000;
    printf("delete锛歵he time is %.2lf us ( %.2f Mops)\n",
           (double) diff, (double) (size) / diff);
...
TEST(TimerTest,TimerinTimer){
    ...
    timer1.Start();
    {
        ...
        timer2.Start();
        {
            ...
            printf("in_in     锛歵he time is %.2lf us ( %.2f Mops)\n",
                   (double) diff, (double) (1) / diff);
        }
        timer2.Stop();
        double diff = timer2.Elapsed() * 1000000;
        printf("in   锛歵he time is %.2lf us ( %.2f Mops)\n",
               (double) diff, (double) (1) / diff);
    }
    timer1.Stop();
    double diff = timer1.Elapsed() * 1000000;
    printf("out 锛歵he time is %.2lf us ( %.2f Mops)\n",
           (double) diff, (double) (1) / diff);
