...
In this section, we conduct extensive experiments by comparing the proposed hash table design \voter with several state-of-the-art approaches for GPU-based hash tables and one CPU-based concurrent hash table. 
...
\begin{itemize}
	...
	\item \revise{\warp is a state-of-the-art warp-centric approach for GPU-based hash table \cite{junger2018warpdrive}. \warp employs a linear probe approach for handling hash collisions. }
...
We exclude the CPU approach as it is significantly slower than the GPU-based approaches according to Figure~\ref{fig:static-all}. 
...
We conduct all experiments on an Intel Xeon E5-2682 Server equipped with NVIDIA Tesla P100-PCIe GPU. Evaluations are performed using CUDA 9.1 on Centos 7.2. The optimization level (-O3) is applied for compiling all programs.
}
...
\begin{table}[b]
	\centering
	\revise{\caption{The number bucket accessed for subtable resizing.}}
	\vspace{-1.5em}
	\label{table:buckets}
	\begin{tabular}{|c|c|c|c|c|}
		...
		\dstwitter & 4741647 & 5645567 & 628398 & 628686 \\ \hline
		\dsreddit & 4738102 & \yc{5656178} & 628403 & 628945 \\ \hline
		\dstpch& 4746638 &  \yc{5656178} & 628332 & 628773\\ \hline
		\dsali& 4736491 & 5648853 & 628291 & 628910 \\ \hline
		\dsrandom& 4731112 & 5645567 & 628448 & 628884 \\ \hline
	\end{tabular}
\end{table}
...
\revise{The performance slightly degrades with more tables beyond 4. As the total size of the allocated memory is fixed,
...
\revise{We record the number of bucket accessed to processes all resizing strategies in Table~\ref{table:buckets}. First, downsize accesses less buckets than that of upsize as there are less number of KV pairs to be moved for downsize (total size of the allocated table is fixed). Furthermore, \voter and rehash access similar number of buckets for downsize as the table is mostly empty at 30\% filled factor. Second, \voter accesses less number of buckets than that of rehash in the upsize scenario. This is because the tables are almost filled for upsize at 85\%, which leads to more cuckoo evictions for rehash whereas no eviction required for \voter. In Figure~\ref{fig:resize}, we also measure the throughput of both methods as the number of KV pairs to be moved over the time to complete resizing. The throughput advantage of \voter cannot be explained purely by the less number of buckets accessed for resizing. In fact, \voter does not need to lock any buckets for upsizing and only needs to lock a small number of buckets for downsizing when eviction, whereas rehash needs to lock every accessed buckets. The experimental results confirm the superiority of our proposed resizing strategy.}
...
\begin{figure}[t]
	...
	\begin{minipage}{0.48\linewidth}\centering
		...
		\centerline{\formal{find}}
	\end{minipage}
	\revise{\caption{Throughput of all compared approaches under the static setting.}}
...
\begin{figure}[t]
	...
	\begin{minipage}{0.48\linewidth}\centering
		...
		\centerline{\formal{find}}
	\end{minipage}
	\revise{\caption{Throughput of all compared approaches for varying the filled factor against the \dsrandom dataset.}}
...
\begin{figure*}[htp]
	...
	\begin{minipage}{0.19\linewidth}\centering
		...
		\centerline{\dsrandom}
	\end{minipage}
	\revise{\caption{Throughput for varying the ratio $r$.}}
...
\begin{figure*}[htp]
	...
	\begin{minipage}{0.19\linewidth}\centering
		...
		\centerline{\dsrandom}
	\end{minipage}
	\revise{\caption{Tracking the filled factor.}}
...
\begin{figure*}[htp]
	...
	\begin{minipage}{0.19\linewidth}\centering
		...
		\centerline{\dsrandom}
	\end{minipage}
	\revise{\caption{Throughput for varying Batch Size .}}
...
\revise{For \slab, the performance of both \formal{insert} and \formal{find} is dramatically affected by the filled factor. This is because \slab employs the chaining approach, where a high filled factor leads to long chains and poor performance.
...
\begin{figure*}[htp]
	...
	\begin{minipage}{0.19\linewidth}\centering
		...
		\centerline{\dsrandom}
	\end{minipage}
	\revise{\caption{Throughput for varying $\alpha$ .}}
...
\begin{figure*}[htp]
	...
	\begin{minipage}{0.19\linewidth}\centering
		...
		\centerline{\dsrandom}
	\end{minipage}
	\revise{\caption{Throughput for varying $\beta$ .}}
