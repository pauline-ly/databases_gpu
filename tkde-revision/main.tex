...
\begin{abstract}
The hash table is a fundamental data structure that
has been implemented on graphics processing units (GPUs) to accelerate a wide range of data analytics workloads. Most existing works have focused on static scenarios and occupying large GPU memory to maximize the insertion efficiency. In many cases, data stored in hash tables get updated dynamically and existing approaches use unnecessarily large memory resources.
...
We devise an efficient resizing strategy for dynamic scenarios without rehashing the entire table that ensures a guaranteed filled factor.
The strategy trades search performance with resizing efficiency, and this tradeoff can be configured by users.
To further improve efficiency, we further propose a two-layer cuckoo hashing scheme that ensures a maximum of \emph{two} lookups for find and delete operations, while retaining similar performance for insertion as general cuckoo hash tables. 
